...
function DataManagementInputKdb() {
  ...
  const onSubmit = (
    ...
    if (building_tag) building_tag = building_tag.map((tag) => tag.value);
    if (persil_tag) persil_tag = persil_tag.map((tag) => tag.value);
    if (pola_ruang_tag) pola_ruang_tag = pola_ruang_tag.map((tag) => tag.value);
...
export default DataManagementInputKdb;
