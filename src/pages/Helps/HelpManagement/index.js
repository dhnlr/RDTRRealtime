...
function HelpManagement() {
  ...
  useEffect(() => {
    if (JSON.parse(Cookies.get("permissions")).indexOf("Dasbor") === -1)
      history.goBack();
  }, [history]);
      ...
      const getUser = async () => {
        try {
          ...
          const datas = await axios.get(
            config.url.API_URL + "/Bantuan/GetList",
            {
              params: {
    ...
    try {
      Swal.fire({
        ...
        if (action.isConfirmed) {
          axios
            .delete(config.url.API_URL + "/Bantuan/Delete", {
              params: {
                id,
              },
            })
                            ...
                            <Table
                              ...
                              columns={[
                                ...
                                },
                                {
                                  ...
                                  Cell: (row) => (
                                    <div>
                                      {String(row.cell.value).slice(0, 50) +
                                        "..."}
                                    </div>
                                  ),
                                },
